...
//Internal bitmask functions, declared with static inline since most of these functions are very small.
...
static inline void bitmask_clear(uint8_t *mask, uint8_t value, uint32_t bitmask_size) {
  memset(mask, value ? 0xFF : 0, bitmask_size);
}

static inline void bitmask_set(uint8_t *mask, uint64_t bit_index, uint8_t value) {
...
static inline uint8_t bitmask_test(uint8_t *mask, uint64_t bit_index) {
  // example
...
static inline void bitmask_and(uint8_t *dest, uint8_t *op1, uint8_t *op2, uint32_t bitmask_size) {
